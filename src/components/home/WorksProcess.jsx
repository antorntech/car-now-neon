import React from "react";

const WorksProcess = () => {
  return (
    <div className="w-full px-5 pt-[80px] lg:pt-[100px]">
      <div className="w-full text-center">
        <div data-aos="fade-up" data-aos-duration="1000">
          <p className="text-3xl md:text-5xl font-bold text-white">
            Rent a Car in Malta <br />
            <span className="text-3xl md:text-5xl font-bold bg-white bg-gradient-to-r from-[#74EE15] via-[#74EE15] to-[#59c107] inline-block text-transparent bg-clip-text">
              and Enjoy
            </span>
          </p>
        </div>
        <p className="w-full md:w-2/3 mx-auto mt-3 text-white">
          We understands that your time is valuable. We offer a hassle-free car
          rental experience that will allow you to focus on your trip to Malta.
          Book today and start planning your adventure.
        </p>
      </div>
      <div className="max-w-screen-xl mx-auto mt-8 md:mt-12 lg:mt-16">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5">
          <div className="relative h-[680px]">
            <div className="absolute top-0 works-card1 w-full h-[370px] z-50 p-5 bg-gradient-to-l from-[#74EE15] to-[#59c107] text-white shadow-lg">
              <h1
                data-aos="fade-right"
                data-aos-delay="300"
                data-aos-easing="ease-in-sine"
                className="text-4xl font-semibold"
              >
                01
              </h1>
              <div
                data-aos="fade-right"
                data-aos-delay="500"
                data-aos-easing="ease-in-sine"
                className="w-12 h-2 bg-white"
              ></div>
              <p
                data-aos="fade-right"
                data-aos-delay="700"
                data-aos-easing="ease-in-sine"
                className="text-2xl lg:text-4xl mt-8 mb-3"
              >
                Choose a<br />{" "}
                <span className="text-2xl lg:text-4xl font-bold">Car</span>
              </p>
              <img
                data-aos="fade-right"
                data-aos-delay="900"
                data-aos-easing="ease-in-sine"
                src="/images/works1.png"
                alt=""
              />
            </div>
            <div className="group overflow-hidden h-[330px] relative mt-[350px]">
              <img
                src="https://queencarentalmalta.com/static/media/choose_car.6ef6cbbf08fd30841ca1.jpg"
                alt=""
                className="w-full h-full group-hover:scale-110 transition-all duration-300"
              />
              <div className="w-full h-full absolute top-0 left-0 bg-black opacity-0 group-hover:opacity-50 transition-all duration-500"></div>
            </div>
          </div>
          <div className="relative h-[680px]">
            <div className="group overflow-hidden h-[320px] relative mb-[340px]">
              <img
                src="https://queencarentalmalta.com/static/media/cards.4c9cb3cde8a29d5eb364.png"
                alt=""
                className="w-full h-full group-hover:scale-110 transition-all duration-300"
              />
              <div className="w-full h-full absolute top-0 left-0 bg-black opacity-0 group-hover:opacity-50 transition-all duration-500"></div>
            </div>
            <div className="absolute bottom-0 works-card2 w-full h-[380px] z-50 p-5 bg-gradient-to-l from-[#74EE15] to-[#59c107] text-white shadow-lg">
              <h1
                data-aos="fade-right"
                data-aos-delay="300"
                data-aos-easing="ease-in-sine"
                className="text-4xl font-semibold mt-12"
              >
                02
              </h1>
              <div
                data-aos="fade-right"
                data-aos-delay="500"
                data-aos-easing="ease-in-sine"
                className="w-12 h-2 bg-white"
              ></div>
              <p
                data-aos="fade-right"
                data-aos-delay="700"
                data-aos-easing="ease-in-sine"
                className="text-2xl lg:text-4xl mt-8 mb-3"
              >
                Pay
                <br />{" "}
                <span className="text-2xl lg:text-4xl font-bold">Online</span>
              </p>
              <img
                data-aos="fade-right"
                data-aos-delay="900"
                data-aos-easing="ease-in-sine"
                src="/images/works2.png"
                alt=""
              />
            </div>
          </div>
          <div className="relative h-[680px]">
            <div className="absolute top-0 works-card1 w-full h-[370px] z-50 p-5 bg-gradient-to-l from-[#74EE15] to-[#59c107] text-white shadow-lg">
              <h1
                data-aos="fade-right"
                data-aos-delay="300"
                data-aos-easing="ease-in-sine"
                className="text-4xl font-semibold"
              >
                03
              </h1>
              <div
                data-aos="fade-right"
                data-aos-delay="500"
                data-aos-easing="ease-in-sine"
                className="w-12 h-2 bg-white"
              ></div>
              <p
                data-aos="fade-right"
                data-aos-delay="700"
                data-aos-easing="ease-in-sine"
                className="text-2xl lg:text-4xl mt-8 mb-3"
              >
                Pick up the
                <br />{" "}
                <span className="text-2xl lg:text-4xl font-bold">Vehicle</span>
              </p>
              <img
                data-aos="fade-right"
                data-aos-delay="900"
                data-aos-easing="ease-in-sine"
                src="/images/works3.png"
                alt=""
              />
            </div>
            <div className="group overflow-hidden h-[330px] relative mt-[350px]">
              <img
                src="https://queencarentalmalta.com/static/media/Group1.9d07e886c0ced3d16980.png"
                alt=""
                className="w-full h-full group-hover:scale-110 transition-all duration-300"
              />
              <div className="w-full h-full absolute top-0 left-0 bg-black opacity-0 group-hover:opacity-50 transition-all duration-500"></div>
            </div>
          </div>
          <div className="relative h-[680px]">
            <div className="group overflow-hidden h-[320px] relative mb-[340px]">
              <img
                src="https://queencarentalmalta.com/static/media/Group.048cd21ce2ad4552bee1.png"
                alt=""
                className="w-full h-full group-hover:scale-110 transition-all duration-300"
              />
              <div className="w-full h-full absolute top-0 left-0 bg-black opacity-0 group-hover:opacity-50 transition-all duration-500"></div>
            </div>
            <div className="absolute bottom-0 works-card2 w-full h-[380px] z-50 p-5 bg-gradient-to-l from-[#74EE15] to-[#59c107] text-white shadow-lg">
              <h1
                data-aos="fade-right"
                data-aos-delay="300"
                data-aos-easing="ease-in-sine"
                className="text-4xl font-semibold mt-12"
              >
                04
              </h1>
              <div
                data-aos="fade-right"
                data-aos-delay="500"
                data-aos-easing="ease-in-sine"
                className="w-12 h-2 bg-white"
              ></div>
              <p
                data-aos="fade-right"
                data-aos-delay="700"
                data-aos-easing="ease-in-sine"
                className="text-2xl lg:text-4xl mt-8 mb-3"
              >
                Discover
                <br />{" "}
                <span className="text-2xl lg:text-4xl font-bold">Malta</span>
              </p>
              <img
                data-aos="fade-right"
                data-aos-delay="900"
                data-aos-easing="ease-in-sine"
                src="/images/works4.png"
                alt=""
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default WorksProcess;
